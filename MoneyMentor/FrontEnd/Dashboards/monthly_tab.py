...
def get_month_file(month, year):
    ...
    return year_folder / f"{month}.json"
...
def create_tab(notebook):
    ...
    # Amount entry
    tk.Label(input_frame, text="Amount:").pack(pady=2)
    ...
    def refresh_list():
        ...
        for entry in current_data:
            ...
            type_value = entry.get("type", "Expense")
            ...
            listbox.insert(
                tk.END,
                f"{type_value}: ${amount:.2f} - {category} ({time_str})"
            )

        if current_data:
            listbox.insert(tk.END, f"--- Balance: ${total:.2f} ---")
    ...
    def save_expense():
        ...
        try:
            if not value:
                result.config(text="Enter an amount.")
                return

            amount = float(value)
            if amount <= 0:
                result.config(text="Amount must be positive.")
                return
    ...
    year_combo.bind("<<ComboboxSelected>>", on_month_change)

    # ===== Save Button =====
    save_button = ttk.Button(container, text="Save Entry", command=save_expense)
